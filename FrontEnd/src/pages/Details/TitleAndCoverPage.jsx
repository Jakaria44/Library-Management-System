...
const TitleAndCoverPage = ({ book, editions }) => {
    ...
    const handleEditionChange = (event) => {
        ...
        const selectedEdition = editions.find((edition) => edition.EDITION === selectedValue);
    ...
    const handleApplyToGet = () => {
        if (["admin", "employee", "user"].includes(localStorage.getItem("role")?.toLowerCase())) {
            apply();
        } else {
            // do some pop up;
            setShowMessage(true);
    ...
    const bookInfoList = [
        ...
        {
            icon: <Language />,
            label: `Language: ${Languages.filter((item) => item.code === book.LANGUAGE)[0].name}`,
            variant: "h3",
        },
        {
            icon: <CalendarMonth />,
        ...
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={8}>
                <Paper elevation={3} sx={{ padding: "16px" }}>
                    ...
                    <Typography variant="h3" style={{ marginTop: "16px", marginLeft: "16px" }}>
                        Edition:
                        <Select
