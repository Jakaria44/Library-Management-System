...
export async function getAllBookSumDB(context) {
    ...
    let query = "SELECT B.ISBN, B.TITLE AS TITLE, B.IMAGE, B.PUBLISH_YEAR AS PUBLISH_YEAR, B.NUMBER_OF_PAGES AS PAGE, B.LANGUAGE, LISTAGG(A.NAME, ', ') AS AUTHORS, NVL(ROUND(AVG(R.RATING), 2), 0) AS RATING, P.NAME AS PUBLISHER";
...
export async function getAllAuthorsDB() {
    let query =
        'SELECT AUTHOR_ID, (P.FIRST_NAME||\' \'||P.LAST_NAME) AS AUTHOR_NAME FROM AUTHOR'
...
export async function getAuthorDB(context) {
    ...
    if (context.AUTHOR_ID) {
        ...
        query +=
            'SELECT AUTHOR_ID, (FIRST_NAME||\' \'||LAST_NAME) AS' +
            ' AUTHOR_NAME, TO_CHAR(DoB,\'DD-MM-YYYY\') AS DoB, TO_CHAR(DoD,\'DD-MM-YYYY\') AS DoD, NATIONALITY, BIO, IMAGE FROM AUTHOR'
...
export async function getAllReviewsOfBookDB(context) {
    let query =
        'SELECT U.USER_ID, (U.FIRST_NAME||\' \'||U.LAST_NAME) AS NAME, R.CONTENT, TO_CHAR(R.EDIT_DATE,\'DD-MM-YYYY\') AS EDIT_DATE, RR.RATING'
    query +=
...
export async function getCompleteBookDB(context) {
    let query =
        'SELECT * FROM BOOK B JOIN PUBLISHER P ON (B.PUBLISHER_ID = P.PUBLISHER_ID)'
    const binds = {}
...
export async function getAdvancedSearchedBookDB(context) {
    ...
    let queryRating =
        'SELECT B.ISBN FROM BOOK B LEFT JOIN RATING R ON (B.ISBN = R.ISBN)'
    queryRating += '\nGROUP BY B.ISBN'
    queryRating +=
    ...
    if (context.YEAR_START) {
        query01 +=
            'B.ORIGINAL_PUBLICATION_YEAR BETWEEN :YEAR_START AND :YEAR_END AND '
    ...
    if (context.SORT) {
        if (context.SORT === 'title') {
            ...
            query01 =
                ...
                '\nFROM (' +
                query01 +
                ') T LEFT JOIN RATING R ON (T.ISBN = R.ISBN)'
            query01 +=
...
export async function addPublisherDB(publisher) {
    const procedure =
        'INSERT_PUBLISHER(:NAME, :ADDRESS, :EMAIL_ID, :WEB_ADDRESS)'
    let query = runProcedure(procedure)
