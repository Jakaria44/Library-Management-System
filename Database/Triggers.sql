...
CREATE OR REPLACE FUNCTION IS_VALID_INSERT_EDITION(A_ISBN VARCHAR2, A_EDITION_NUM NUMBER) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
CREATE OR REPLACE FUNCTION IS_VALID_EDITION(A_ISBN VARCHAR2, A_EDITION_NUM NUMBER) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
END;
/
CREATE OR REPLACE FUNCTION IS_VALID_INSERT_JOB(A_TITLE IN VARCHAR2) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
CREATE OR REPLACE FUNCTION IS_VALID_UPDATE_JOB(J_ID VARCHAR2, J_TITLE IN VARCHAR2) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
CREATE OR REPLACE FUNCTION IS_VALID_DELETE_JOB(J_ID VARCHAR2) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
CREATE OR REPLACE FUNCTION IS_VALID_DELETE_ADMIN(A_ID IN VARCHAR2) RETURN BOOLEAN IS
    COUNTER NUMBER;
BEGIN
    SELECT COUNT(*)
    INTO COUNTER
...
BEGIN
    IF
        (IS_VALID_INSERT_EDITION(A_ISBN, A_EDITION_NUM)) THEN
...
BEGIN
    ...
    ELSIF COUNTER = 0 THEN
        INSERT INTO EMPLOYEE(USER_ID, JOB_ID, JOIN_DATE, END_DATE)
        VALUES (A_USER_ID, A_JOB_ID, SYSDATE, NULL);
    end if;
    DELETE
    FROM APPLY
    WHERE USER_ID = A_USER_ID;
...
CREATE OR REPLACE PROCEDURE END_EMPLOYEE(A_USER_ID VARCHAR2) IS
    ID VARCHAR2(2000);
BEGIN
    SELECT USER_ID
    INTO ID
...
BEGIN
    IF
        (IS_VALID_EDITION(A_ISBN, A_EDITION_NUM)) THEN
        UPDATE EDITION
        SET NUM_OF_COPIES = NUM_OF_COPIES + A_NUM_OF_COPIES,
...
CREATE OR REPLACE PROCEDURE EDIT_RENT_HISTORY(A_ID VARCHAR2) IS
    ID VARCHAR2(200);
BEGIN
...
BEGIN
    ...
    WHERE Rent_History_ID = A_RENT_ID
      AND STATUS = 0 AND Return_Date < SYSDATE;
    IF COUNTER = 1 THEN
...
BEGIN
    ...
    INTO ID
    FROM FINE_HISTORY
    WHERE Rent_History_ID = A_ID AND Payment_Date IS NULL;
    UPDATE FINE_HISTORY
