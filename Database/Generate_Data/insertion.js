...
(async () => {
  ...
  try {
    ...
    async function getAuthorId(authorName) {
      let result = await connection.execute(
        'SELECT Author_ID FROM AUTHOR WHERE UPPER(Name) = UPPER(:name)',
        {name: authorName}
      );
      if (result.rows.length === 0) {
        const a = 
        {name: authorName, dob:new Date(`${Math.floor( 1800 + Math.random()*200)}-01-01`)};
        console.log( a);
        await connection.execute(
    ...
    async function getGenreId(genreName) {
      let result = await connection.execute(
        'SELECT Genre_ID FROM GENRE WHERE UPPER(Genre_Name) = UPPER(:name)',
        {name: genreName}
      );
      if (result.rows.length === 0) {
        await connection.execute(
          'INSERT INTO GENRE (Genre_Name) VALUES (INITCAP((LOWER(:name))))',
          {name: genreName}
        );
    ...
    for (const bookData of jsonData) {
      ...
      ||!await check_isbn(String(bookData['industryIdentifiers'][0]['identifier'])) 
      || bookData.language === 'ko'
      ) {
        continue;
      }
  const isbn = String(bookData['industryIdentifiers'][0]['identifier']);
      ...
      await connection.execute(
        ...
        {
          isbn,
          title: bookData.title.slice(0, 490),
          image: bookData.imageLinks.thumbnail,
          pages: bookData.pageCount,
          language: bookData.language,
          description: bookData.description.slice(0,2990),
